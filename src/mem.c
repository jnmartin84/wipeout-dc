...
void *mem_bump(uint32_t size) {
	error_if(bump_len + temp_len + size >= MEM_HUNK_BYTES, "Failed to allocate %lu bytes in hunk mem", size);
...
void *mem_temp_alloc(uint32_t size) {
	...
	error_if(bump_len + temp_len + size >= MEM_HUNK_BYTES, "Failed to allocate %lu bytes in temp mem", size);
...
void mem_temp_check(void) {
	error_if(temp_len != 0, "Temp memory not free: %lu object(s)", temp_objects_len);
}
