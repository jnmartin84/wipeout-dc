...
void ship_ai_update_intro_await_go(ship_t *self) {
	self->position.y = self->temp_target.y + sinf(self->update_timer * (80.0 + self->pilot * 3.0) * 0.04601942363656923689154262768476f/* 30.0 * twopi_i754 / 4096.0 */) * 32;
...
void ship_ai_update_race(ship_t *self) {
	...
	if (flags_not(self->flags, SHIP_FLYING)) {
		...
		track_target = vec3_mulf(track_target, self->speed * approx_recip(gap_length));
		...
		self->acceleration = vec3_add(self->acceleration, vec3_mulf(vec3_sub(
			vec3_mulf(face->normal, (SHIP_TRACK_FLOAT * SHIP_TRACK_MAGNET) * approx_recip(height)), /// height),
		...
		self->angular_velocity.x = wrap_angle(-bump_atan2f(track_target.y, xy_dist) - self->angle.x) * 1.875f;//(1.0/16.0) * 30;
	...
	// Ship is SHIP_FLYING
		...
		float recip_gap_length = approx_recip(vec3_len(track_target));

		track_target.x = (track_target.x * self->speed) * recip_gap_length; /* / gap_length; */
		track_target.z = (track_target.z * self->speed) * recip_gap_length;/* / gap_length; */
